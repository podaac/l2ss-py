...
def remove_duplicate_dims(nc_dataset):
    ...
    for dup_var_name, dup_var in dup_vars.items():
        ...
        dim_size = nc_dataset.dimensions[dim_dup].size
        ...
        data[dim_dup_new] = nc_dataset.createVariable(dim_dup_new, nc_dataset.variables[dim_dup].dtype,\
                                                          (dim_dup_new,), fill_value=dup_var._FillValue)
        ...
        data[var_name_new] = nc_dataset.createVariable(var_name_new, str(dup_var[:].dtype), tuple(new_dim_list), fill_value=dup_var._FillValue)
